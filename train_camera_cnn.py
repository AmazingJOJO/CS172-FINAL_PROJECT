...
def training(epochs = 200):
    ...
    pretrained_dict = {k: v for k, v in model_dict.items() if k in my_dict}
    ...
    for epoch in range(epochs):
        ...
        if accu > best_accu:
            best_accu = accu
            torch.save(model,'argu/best_model_50.pkl')
            print("accuracy is ",accu)
    torch.save(model,'argu/model_50.pkl')
...
'''
model = torch.load('argu/model_50.pkl')
dev = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
model = model.to(dev)
